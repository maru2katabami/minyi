...
export function MiniPlay({children}) {
  ...
  return (
    <div
      ...
      {isOpen ?
      <div
        className="absolute px-4 top-0 w-full h-10 flex justify-between items-center">
        <div
          className="space-x-2 flex items-center">
          <div
            id="close"
            onClick={handleClick}
            className="size-3 border border-red-500 bg-red-400 rounded-xl"/>
          <div
            className="size-3 border border-yellow-500 bg-yellow-400 rounded-xl"/>
          <div
            className="size-3 border border-green-500 bg-green-400 rounded-xl"/>
        </div>
        {children}
      </div>:
      <div
        id="open"
        className="size-full bg-[url(/minyi.png)] bg-no-repeat bg-center bg-[size:40px]"
        onClick={handleClick}/>
      }
    </div>
  )
